...
A hands-on network forensics project demonstrating real-world SOC analyst skills — live packet capture, traffic baselining, threat detection, and full incident triage of an active malware infection using Wireshark.
...
- **PCAP Source:** [malware-traffic-analysis.net](https://malware-traffic-analysis.net) — trusted threat intelligence resource used by security professionals globally
...
## Part 1 — Live Capture & Traffic Baselining

Captured live traffic on a Linux host while generating real network activity via ping, HTTP requests, and an nmap service scan. Analyzed protocol distribution and top-talking hosts to establish a clean baseline — a critical reference point for anomaly detection.
...
- TLS at 10.1% — confirmed encrypted web traffic
- No cleartext credentials detected — all traffic over HTTPS
- Clean ARP behavior — no poisoning indicators
...
Applied targeted Wireshark filters to isolate specific traffic types — the same workflow used in real SOC environments to cut through noise and find threats fast.
...
| `icmp` | Ping sweep / flood detection | 23 packets — clean baseline behavior |
...
## Part 3 — Malware PCAP Forensics & Incident Triage

The core of this project. Analyzed a real-world malware capture from [malware-traffic-analysis.net](https://malware-traffic-analysis.net) containing active **NetSupport Manager RAT** traffic — a remote access trojan seen in real enterprise incidents in 2026.
...
nbns
```
NetBIOS name registration packets revealed the Windows hostname. MAC address extracted from Ethernet frame headers.
...
I'm building a hands-on portfolio in cybersecurity and cloud security, focused on real-world skills that matter in security operations, threat detection, and incident response. This lab is one piece of a larger body of work documenting my technical progression.

Connect with me on [LinkedIn](#) | Follow my work on [X/Twitter](#)
